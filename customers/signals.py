...
from .models import Order, Orderitem, PushSubscription
...
def notify_admin_new_order(sender, instance, created, **kwargs):
    ...
    Only acts on brand-new unshipped orders.
    Sends:
      1. Email to every superuser address.
      2. Web Push notification to every subscribed admin browser.
    """
    if not created:
        return
    ...
    _send_order_email(instance)
    _send_order_push(instance)
...
def _send_order_push(order):
    """Send a Web Push notification to every stored admin browser subscription."""
    try:
    ...
    if not vapid_private or not vapid_claims:
        logger.warning(
            'MDF notifications: VAPID_PRIVATE_KEY / VAPID_CLAIMS not set in settings — skipping push.'
        )
        return
